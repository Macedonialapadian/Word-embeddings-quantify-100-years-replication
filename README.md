...
- **Output folder:** `output/`
  ...
  - `output/plots/extend_figure/` - Extended figure using Newsroom embeddings
  - `output/regressions/` - Regression results generated by plotting scripts

- **Extended Figure Data:** The 'data' folder under `output/plots/extend_figure/` is empty, it should be placed with 'newsroom_test.jsonl' files download from this [link](https://drive.google.com/drive/folders/1OpWGtNB-IJTg4nE6U8yRb41mKaPVk-oP?usp=drive_link), and it is originally come from this [project](https://lil.nlp.cornell.edu/newsroom/index.html)
...
6. The replace plot for NYT data
  - In the `code/` folder, run the jupyter notebook `Embeddings_extend_figure.ipynb` to get the extended figure that uses Newsroom_Embeddings as replacement for NYT embeddings.
